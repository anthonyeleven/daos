...
const PoolMetadataSize uint64 = uint64(200)*uint64(humanize.MiByte)
...
func (cmd *PoolCreateCmd) GetMaxPoolSize(ctx context.Context) (scmBytes uint64,
	...
	for _, key := range hostStorageMap.Keys() {
		...
		for _, scmNamespace := range hostStorage.ScmNamespaces {
			...
			if scmNamespaceFreeBytes < PoolMetadataSize {
				missingBytes := PoolMetadataSize-scmNamespaceFreeBytes
				...
				msg = fmt.Sprintf(msg,
				                  ...
				                  hostSet.String(),
				                  humanize.Bytes(PoolMetadataSize),
