...
class PerformanceTestBase(IorTestBase, MdtestBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Need to restart so daos_metrics is fresh
    ...
    @property
    def unique_id(self):
        """A unique id for each test case run."""
    ...
    def run_performance_ior(self, namespace=None, use_intercept=True, stop_delay_write=None, stop_delay_read=None):
        ...
        stop_after_write = (stop_delay_write or 0) * (self.ior_cmd.sw_deadline.value or 0)
        stop_after_read = (stop_delay_read or 0) * (self.ior_cmd.sw_deadline.value or 0)
        ...
        if stop_after_write:
            time.sleep(stop_after_write)
            rank_to_stop = self.num_servers - 1
            self.server_managers[0].stop_ranks([rank_to_stop], self.d_log, force=True)
        ...
        if stop_after_read:
            time.sleep(stop_after_read)
            rank_to_stop = self.num_servers - 2
            self.server_managers[0].stop_ranks([rank_to_stop], self.d_log, force=True)
        ior_returncode = self.job_manager.process.wait()
        try:
            if ior_returncode != 0:
                self.fail("IOR read failed")
    ...
    def run_performance_mdtest(self, namespace=None, stop_delay=None):
        ...
        Args:
            ...
            stop_delay (float, optional): fraction of stonewall time after which to stop a
                rank during. Must be between 0 and 1. Default is None.
        ...
        stop_after = (stop_delay or 0) * (self.mdtest_cmd.stonewall_timer.value or 0)

        self._log_performance_params("mdtest", [["MDTEST Flags", self.mdtest_cmd.flags.value]])
        ...
        self.container.chunk_size.update(self.mdtest_cmd.dfs_chunk.value)
